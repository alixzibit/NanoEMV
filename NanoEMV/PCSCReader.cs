...
namespace NanoEMV
{
    public class PCSCReader : IDisposable
    {
        ...
        public void ColdReset()
        {
            ...
            // Assuming 'card' is your card handle and it's defined at class level
            int result = WinSCard.SCardReconnect(card,
                (uint)WinSCard.ShareMode.SCARD_SHARE_SHARED,
                (uint)WinSCard.Protocol.SCARD_PROTOCOL_T0,
                (uint)WinSCard.Disposition.SCARD_RESET_CARD,
                out activeProtocol);


            if (result != WinSCard.SCARD_S_SUCCESS)
            {
                string errorMessage = $"Scard Error - {result.ToString()}";
        ...
        public void WarmReset()
        {
            int result = WinSCard.SCardReconnect(card,
                (uint)WinSCard.ShareMode.SCARD_SHARE_SHARED,
                (uint)WinSCard.Protocol.SCARD_PROTOCOL_T0 | (uint)WinSCard.Protocol.SCARD_PROTOCOL_T1,
                (uint)WinSCard.Disposition.SCARD_RESET_CARD,
                out activeProtocol);
